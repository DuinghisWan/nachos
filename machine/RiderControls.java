...
public interface RiderControls {
    /**
     * Return the number of floors in the elevator bank. If <i>n</i> is the number
     * of floors in the bank, then the floors are numbered <i>0</i> (the ground
     * floor) through <i>n - 1</i> (the top floor).
     *
    ...
    /**
     * Set the rider's interrupt handler. This handler will be called when the rider
     * observes an event.
     *
     ...
     * floor the rider started on, nor does it count floors where the rider is in
     * the elevator and does not exit.
     *
     ...
     *
     * @param up <tt>true</tt> to press the up button, <tt>false</tt> to press the
     *           down button.
    ...
    /**
     * Press the up button. The rider must not be on the top floor and must not be
     * inside an elevator. If an elevator is on the same floor as this rider, has
     * the doors open, and says it is going up, does nothing and returns
     * <tt>false</tt>.
     *
    ...
    /**
     * Press the down button. The rider must not be on the bottom floor and must not
     * be inside an elevator. If an elevator is on the same floor as as this rider,
     * has the doors open, and says it is going down, does nothing and returns
     * <tt>false</tt>.
     *
     * @return <tt>true</tt> if the button event was sent to the elevator
     *         controller.
    ...
    /**
     * Enter an elevator. A rider cannot enter an elevator if its doors are not open
     * at the same floor, or if the elevator is full. The rider must not already be
     * in an elevator.
     *
    ...
    /**
     * Press a floor button. The rider must be inside an elevator. If the elevator
     * already has its doors open on <tt>floor</tt>, does nothing and returns
     * <tt>false</tt>.
     *
    ...
    /**
     * Exit the elevator. A rider cannot exit the elevator if its doors are not open
     * on the requested floor. The rider must already be in an elevator.
     *
     ...
     * events pending when a rider interrupt occurs, so this method should be called
     * repeatedly until it returns <tt>null</tt>.
     *
     * @return the next event, or <tt>null</tt> if no further events are currently
     *         pending.
     */
